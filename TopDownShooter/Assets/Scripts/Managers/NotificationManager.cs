...
public static class NotificationsManager 
{
	...
	private static Dictionary<string, List<Component>> Listeners = new Dictionary<string, List<Component>>();
	
	//Methods
	//------------------------------------------------
	//Function to add a listener for an notification to the listeners list
	public static void AddListener(Component Sender, string NotificationName)
	{
	...
	public static void RemoveListener(Component Sender, string NotificationName)
	{
	...
	public static void PostNotification(Component Sender, string NotificationName)
	{
		//If no key in dictionary exists, then exit
		if(!Listeners.ContainsKey(NotificationName))
			return;
	...
	private static void OnLevelWasLoaded()
	{
		//Clear redundancies
		RemoveRedundancies();
	}
